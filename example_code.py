...
from nbconvert.preprocessors import base, Preprocessor
...
class Preprocessor(NbConvertBase):
    ...
    Inherit from this class if you wish to have configurabil
    preprocessor.

    Any configurable traitlets this class exposed will be co
    profiles using c.SubClassName.attribute = value

    You can overwrite `preprocess_cell()` to apply a transfo
    independently on each cell or `preprocess()` if you pref
    ...
    def __call__(self, nb, resources):
        ...
        if self.enabled:
            self.log.debug("Applying preprocessor: %s", self
    ...
    def preprocess(self, nb, resources):
        ...
        If you wish to apply your preprocessing to each cell
        ...
        resources : dictionary
            Additional resources used in the conversion proc
            preprocessors to pass variables into the Jinja e
        """
        for index, cell in enumerate(nb.cells):
            nb.cells[index], resources = self.preprocess_cel
        return nb, resources

    def preprocess_cell(self, cell, resources, index):
        """
        Override if you want to apply some preprocessing to 
...
# 
    def from_notebook_node(self, nb, resources=None, **kw):
        output, resources = super().from_notebook_node(nb, r
        
        # Apply black formatter if format_black is set to Tr
        # if self.format_black:
            # output = black.format_str(output, mode=black.F
        # 
        # Apply isort to sort imports if sort_imports is set
